...
if($_SERVER["REQUEST_METHOD"] === "GET"){
    ...
    if(array_key_exists("id",$_GET)){
        ...
        try{
            ...
            $query = $connection->prepare('SELECT * FROM citas WHERE id = :id');
        ...
        try{
            $profe = $_SESSION["username"];
            $query = $connection->prepare('SELECT * FROM citas WHERE profe = :profe');//Se le cambia el nombre ya sea photocard, cds, etc
    ...
    if(array_key_exists("_method",$_POST)){//Si existe algo en el formulario name="fecha"
...
function deleteProduct($id,$redirect){
    ...
    try{
        $query = $connection->prepare('UPDATE citas SET active = 0 WHERE id = :id');//Para actualizar es con una coma
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->execute();
